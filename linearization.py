...
from runAstra_3boosters import getGunFunction, getFitParams, runAstraFunction, runAstraFunction2, runAstraCombination
...
def emittanceForBunchMinimum(phase,amplitude,focus,B1_pha,B1_ampl,B2_pha,B2_ampl,Gun_pha,Gun_ampl,bounds_ampl,array_for_values):
    amplitude_for_focus = minimize(runAstraFunction,amplitude,args=(phase,focus,B1_pha,B1_ampl,B2_pha,B2_ampl,Gun_pha,Gun_ampl),method='SLSQP',bounds=bounds_ampl,options={'eps':0.1})
...
def linearizer2order(amplitude_gun,phase_gun,phase_b2):
    ...
    amplitude_booster1 = 10.0e6
    phase_booster1 = 218.5*np.pi/180 #(remember that in Astra the phase is this-90deg)
    amplitude_booster2 = 10.0e6
    phase_booster2 = 110*np.pi/180 #(remember that in Astra the phase is this-90deg)
    booster3_phase_range=[-90.0,90.0]
    booster3_amplitude_range=[5.0e6,11.0e6]
...
#    fig2.savefig('Solutions_'+str(Gun_phase)+'deg_'+str(Gun_amplitude)+'MV.pdf')
    ...
    result_for_minimum = minimize(runAstraCombination, [phase_astra_seed,amplitude_astra_seed], args=(z_focus,phase_gun,amplitude_gun,Booster1_phase,Booster1_amplitude,Booster2_phase,Booster2_amplitude),method='SLSQP',bounds=[phase_bounds,amplitude_bounds],options={'eps':0.01})
    phase_for_minimal = result_for_minimum.x[0]
    amplitude_for_minimal = result_for_minimum.x[1]
...
phases_b2 = [63.0]#np.linspace(270.0,276.0,13)
